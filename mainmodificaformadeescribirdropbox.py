...
def get_average_sensors():
    for index in ad_sensors:
        average_sensors[index]=ad_sensors[index]/ad_count[index]
    #log(average_sensors)
    return average_sensors
    
def write_to_dropbox():
    global moment, ad_sensors, ad_count
    moment = get_moment()
    ...
    if(moment[5]==59):
        average_sensors=get_average_sensors()
        print("waa")# at 59 second open new file with date of all second 
        f = open ("base.txt", "a")
        f.write(str(time.strftime("%Y;%m;%d;%H;%M;%S;",moment)))
        for index in average_sensors:
            f.write(str(int(average_sensors[index]))+";")       
        f.write("\n")
        print(ad_sensors)
        print(ad_count)
